...
| Uses existing auth, no app login | Paginated GraphQL enrichment and PR diff patch context use direct HTTPS requests with `GH_TOKEN` / `GITHUB_TOKEN` or the token from `gh auth token`; base PR/issue view still needs migration off `gh pr/issue view`; auth failures point to `gh auth status` / `gh auth login` |
...
direct GitHub API calls. Paginated GraphQL enrichment and PR diff patch context
are direct HTTPS, but the base PR/issue view still uses `gh` and must move next.
