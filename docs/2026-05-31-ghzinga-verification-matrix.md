...
| Settings and config persistence | `docs/2026-06-01-ghzinga-settings-config-plan.md` documents `~/.config/ghzinga/config.toml`, `XDG_CONFIG_HOME`, `GZG_CONFIG_PATH`, CLI override precedence, and the in-app settings flow; config tests verify missing-file defaults, TOML parsing, invalid-value diagnostics, and save output; reducer/render tests verify `s`, footer `[settings]`, theme/symbol selection rows, and save intents |
