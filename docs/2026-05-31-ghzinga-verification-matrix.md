...
GH_TOKEN= GITHUB_TOKEN= PATH="$(mktemp -d)" target/debug/gzg openclaw/openclaw#81834 --tab overview --once --refresh-seconds 0
GH_TOKEN= GITHUB_TOKEN= PATH="$(mktemp -d)" target/debug/gzg https://github.com/openclaw/openclaw/issues/88499 --tab overview --once --refresh-seconds 0
```
