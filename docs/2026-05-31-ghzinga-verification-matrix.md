...
| Automated quality gate | `scripts/ci-local.sh` runs `cargo fmt --check`, `cargo test`, `cargo clippy --all-targets --all-features -- -D warnings`, `scripts/verify-install.sh`, `sh -n scripts/live-smoke.sh`, the no-network `GZG_LIVE_SELF_TEST=1 scripts/live-smoke.sh` harness self-test, `npx -y @simpledoc/simpledoc check`, `scripts/verify-no-png-captures.sh`, the main PR/issue capture validator self-test, validate-only PR/issue capture evidence checks, the PR and issue mouse-smoke validator self-tests, and validate-only tmux mouse-smoke evidence checks; `.github/workflows/ci.yml` installs Rust/Node and runs the same script on pull requests, pushes to `main`, and manual `workflow_dispatch` runs when GitHub Actions is enabled for the repository; the PNG guard rejects both tracked and generated PNG artifacts under `captures/` so UX evidence stays in terminal text and ANSI transcripts |
| Opt-in live smoke | `scripts/live-smoke.sh` builds `target/debug/gzg` if needed, renders live public PR Overview/Activity/Commits/Checks/Files/Links tabs and live public issue Overview/Activity/Links tabs through the normal direct GitHub HTTP path, then reruns every public PR tab and public issue Overview/Activity/Links with blank `GH_TOKEN`/`GITHUB_TOKEN` and `PATH` pointed at an empty directory so the app cannot use `gh auth token`. It validates high-signal markers in both authenticated-or-existing-token mode and no-login public fallback mode, and its diagnostics are covered by a no-network self-test in CI so shell portability regressions are caught before an opt-in network run. No-login public fallback is skipped by default after the first GitHub shared unauthenticated IP quota exhaustion response, and `GZG_LIVE_REQUIRE_PUBLIC_FALLBACK=1` makes that condition fatal when strict fallback evidence is required. It is not part of CI because network availability, public target state, and GitHub API quota are external inputs. `GZG_LIVE_PR_TARGET` and `GZG_LIVE_ISSUE_TARGET` can override the defaults. |
...
| Mouse capture with opt-out | `TerminalGuard::enter(mouse_enabled)` enables `EnableMouseCapture`; CLI exposes `--no-mouse`; `TerminalGuard` tracks raw mode, alternate screen, and mouse capture independently so partial setup failures still restore only the terminal features that were enabled; `terminal_state_snapshot_and_clear_is_idempotent` and `terminal_state_tracks_features_independently` verify the restoration state machine, and `terminal_guard_restores_before_panic_output` verifies the herdr-style panic hook restores terminal state before delegating to default panic output |
| Uses existing auth, no app login | Base PR/issue fetches, paginated GraphQL enrichment, PR diff patch context, and the `src/github/public_rest.rs` unauthenticated fallback use direct HTTPS requests with `GH_TOKEN` / `GITHUB_TOKEN` or the token from `gh auth token`; auth tests verify token precedence, whitespace trimming, blank-token fallback from `GH_TOKEN` to `GITHUB_TOKEN`, successful trimmed `gh auth token` output, empty `gh auth token` rejection, unavailable-auth classification for GitHub CLI login failures, and public REST fallback classification for clear token-rejection responses such as HTTP 401, Bad credentials, SAML SSO authorization rejection, resource-not-accessible token errors, and GitHub rate-limit errors; public PRs/issues fall back to unauthenticated REST when local credentials are unavailable, clearly rejected, or rate-limited, and public REST comments, PR reviews, PR review comments, timeline events, PR commits, changed files, public check runs, public check suites, head-commit deployments, and legacy status contexts are loaded without auth where GitHub exposes them publicly; normal GraphQL mode requests cheap `hasNextPage` markers on first-page base collections and warns when a section needs `--api-depth full` or `GZG_API_DEPTH=full` for exhaustive pagination; `docs/2026-06-01-ghzinga-http-transport-resilience-plan.md` documents the reusable reqwest client and 30-second per-request timeout; `graphql_transport_receives_post_shape_and_returns_body`, `graphql_budget_preflight_allows_query_when_remaining`, `graphql_budget_preflight_skips_post_when_exhausted`, `rest_transport_receives_get_shape_and_returns_body`, `rest_transport_can_omit_authorization_for_public_requests`, `public_rest_pr_fallback_renders_core_monitoring_surfaces`, `pr_fallback_normalizer_tolerates_missing_optional_fields`, `public_rest_fallback_warning_describes_auth_unavailable`, `public_rest_fallback_warning_describes_token_api_errors`, `base_pr_query_requests_partial_depth_markers`, `base_issue_query_requests_partial_depth_markers`, `base_pr_partial_depth_warning_lists_paginated_sections`, `base_issue_partial_depth_warning_lists_paginated_sections`, `api_depth_parses_user_facing_names`, `parses_api_depth`, `public_rest_issue_fallback_renders_core_monitoring_surfaces`, `pages_until_short_page_without_auth`, `public_rest_fallback_includes_rate_limited_token_errors`, `public_rest_reviews_without_auth`, `public_rest_review_comments_without_auth`, `public_rest_timeline_events_without_auth`, `public_rest_check_runs_page_without_auth_and_preserve_urls`, `public_rest_check_suites_page_without_auth_and_preserve_suite_rollups`, `public_rest_head_deployments_without_auth_and_preserve_latest_status_urls`, `public_rest_status_contexts_without_auth`, `graphql_transport_errors_on_graphql_errors_payload`, and `rest_transport_includes_status_and_body_on_http_failure` verify the mockable HTTP transport boundary, timeout propagation, explicit API-depth control, and public fallback |
| PR body, labels, reactions, author, state, branches | Overview/status render tests verify the fixed status band highlights PR/issue state with a background badge and shows branch direction, checks, and gh-dash-style changed-file totals such as `22 files changed +1.3k -195` without comment/review/reaction/thread/warning counters; live `cargo run --bin gzg -- openclaw/openclaw#81834 --once`; `labels_query_uses_selector_and_pagination_state`, `labels_page_preserves_pagination_state_and_filters_blank_values`, `assignees_query_uses_selector_and_pagination_state`, and `assignees_page_preserves_pagination_state_and_display_names` verify paginated label and assignee enrichment for PR/issue selectors |
...
| PR changed files and patch context | `changed_files_from_graphql_keep_change_type`, `pr_diff_uses_rest_pull_diff_path`, `parses_unified_diff_patches_by_file_path`, `expanded_file_rows_show_patch_context`, `long_patch_rows_are_click_expandable`, `file_rows_style_diff_lines_by_change_kind`, `diff_rows_hide_change_markers_and_preserve_indentation`, PR files captures; patch context uses the direct REST pull-request diff media type, file rows are individually expandable, long patches have their own `[+ more patch]` / `[- less patch]` controls, default diff styling uses green background tint for additions and red background tint for deletions, hunk headers use an accent color, and patch code rows hide raw unified-diff change markers while preserving indentation |
...
| Exact detail URL open targets | `render_registers_exact_comment_url_as_open_url`, `activity_permalink_details_are_clickable_without_expansion`, `check_rows_are_click_expandable`, `expanded_commit_rows_show_deployments`, `keyboard_enter_opens_first_visible_url_action`, `mouse_click_on_url_target_requests_open_url`, `url_open_command_uses_browser_env_when_available`; the PR and issue tmux mouse-smoke captures inject fixture permalinks, click Activity `[details]`, and verify the capture-local browser adapter receives each exact URL |
...
| Live startup and auto-refresh | `docs/2026-06-02-ghzinga-startup-progressive-loading-plan.md` documents the immediate-startup placeholder and initial background fetch, and `docs/2026-06-06-ghzinga-progressive-resource-loading-spec.md` documents the base-first progressive loading model; `loading_placeholder_for_ambiguous_resource_shows_pr_shell`, `loading_placeholder_for_issue_url_keeps_issue_shell`, `initial_fetch_outcome_replaces_placeholder_with_loaded_resource`, and `initial_live_fetch_starts_before_first_draw` verify the startup path without live network calls; `progressive_base_outcome_finishes_blocking_load_before_enrichment` verifies base data replaces the loading shell and clears blocking loading before enrichment arrives; `progressive_enrichment_is_ignored_after_resource_replacement` verifies stale enrichment cannot corrupt a replaced tab; `progressive_enrichment_error_adds_warning_without_failing_resource` verifies section-style background errors do not discard base content; `files_tab_schedules_missing_file_patches_on_demand` and `file_patch_loading_waits_for_files_tab_missing_patches_and_idle_load_state` verify PR diff patch context is scheduled only when the Files tab is active, patches are missing, and the base/enrichment phase is idle; `open_resource_new_tab_shows_loading_tab_immediately` and `replace_resource_shows_loading_placeholder_immediately` verify in-app opens show a loading placeholder before GitHub returns; `auto_refresh_due_requires_live_mode_positive_interval_and_elapsed_time`, `automatic_refresh_starts_background_fetch_and_records_completed_changes`, `automatic_refresh_waits_until_interval_is_due`, and `automatic_refresh_throttles_due_attempt_while_fetch_is_in_progress` verify the event-loop interval predicate, single-flight throttling, and the gateway-backed refresh path; `apply_refreshed_resource` tests preserve tab/scroll and record changed/no-change state; `renders_last_refresh_changed_sections` verifies changed surfaces render in the status panel; `loading_indicator_advances_through_ascii_frames` and `renders_loading_state_in_status_and_footer` verify the terminal-safe loading marker; `fingerprint_changes_when_activity_content_or_metadata_changes` verifies activity body and metadata changes are part of change detection |
| Manual refresh | reducer tests for `r` and `[🔄 refresh]`, footer render-to-click tests |
| Human-readable timestamps | `src/render/time.rs` keeps gh-dash-style compact relative labels such as `30s`, `1h`, `2d`, `3w`, and `2mo` centralized in the render layer; formatter tests cover RFC3339 `Z` timestamps, fractional seconds, timezone offsets, future times, and preformatted fixture labels; render tests verify expanded commits and checks no longer expose raw ISO timestamps; PR and issue terminal captures show compact dates in headers, overview timelines, activity rows, commits, check details, and deployment rows while the domain/API layer keeps raw strings for sorting and fingerprints |
| On-demand full-depth loading | `docs/2026-06-01-ghzinga-on-demand-full-depth-plan.md` documents the in-TUI escape hatch for partial pagination warnings; `detects_partial_depth_warning_marker` keeps the marker centralized, `keyboard_f_loads_full_depth_only_when_partial_warning_is_present` and `mouse_click_on_load_full_target_requests_full_depth_load` verify the reducer path, `footer_shows_load_full_only_for_partial_depth_resources` verifies conditional `[⬇ full]` footer visibility and ordering after `[➕ expand ]`, `full_depth_fetch_outcome_preserves_view_and_reports_loaded_status` verifies the full-depth fetch outcome preserves the active view and clears the partial warning, `full_depth_intent_skips_offline_fixture_mode` verifies fixture runs do not pretend to call GitHub, and the PR tmux mouse-smoke capture starts an isolated partial-depth fixture session, clicks footer `[⬇ full]`, and validates the offline full-depth skip status |
| Visible truncation/expansion | `[➕ more]` / `[➖ less]` render tests for body, activity, commits, checks, and files; untruncated body, activity, and patch rows do not show no-op more/less controls; `[➕ expand ]` / `[➖ collapse]` reducer and render-to-click tests verify tab-level expansion uses visible bottom controls immediately after refresh and does not displace the first feed item |
| Mouse and keyboard routing | synthetic Crossterm `MouseEvent` and `KeyEvent` tests in `src/app/update.rs`, render-to-click tests in `src/render/resource.rs`, and PR/issue evidence under `captures/ghzinga-pr-81834/mouse-smoke/` and `captures/ghzinga-issue-88499/mouse-smoke/` generated by sending real xterm SGR mouse events and keyboard keys into the app inside tmux; render-to-click tests prove both issue references and absolute PR links produce `Navigate` intents; the PR smoke run uses fixture-backed linked-resource loading to prove a clicked issue row replaces the current TUI view and Backspace restores the original PR, while also pressing `a` on Files to expand/collapse every file row; the issue smoke run proves issue body expansion/collapse, Activity `[details]` exact URL opening, keyboard copy routing, linked-issue navigation, Backspace restoration, and `a` on Overview for all-row expansion/collapse; the validators reject duplicate or unnamed capture frames so a broken manifest cannot silently overwrite input evidence |
| Keyboard shortcuts avoid tmux/herdr conflicts | reducer supports arrows, Vim-style `h`/`j`/`k`/`l` arrow aliases, PageUp/PageDown, Home/End, Tab/Shift-Tab and Ctrl-i fallback for PR/issue resource tabs, Shift-Left/Shift-Right for PR/issue resource tabs, visible-position tab jumps with `1`-`6`, Enter for first visible content action, `r`, `f` for full-depth loading when available, `?`, `q`, `o`, `x`, `y`, `v` for feed order, `a` for current-tab expand/collapse-all, Backspace; Left/Right continue switching Overview/Activity/etc. content tabs while Tab/Shift-Tab switch between open resources; `q` closes the active modal/help/settings layer before showing a quit confirmation, while Ctrl-c remains the immediate quit path outside text prompts; `o` opens the resource parser in replace-current mode and `n` opens it in new-tab mode; `keyboard_x_closes_current_resource_tab_only_in_normal_view` and `keyboard_x_keeps_last_resource_tab_open` verify `x` closes only the active resource tab in normal view and never removes the last tab; plain-letter shortcuts ignore Control/Alt modifiers, with tests proving Ctrl-a/Ctrl-b/Ctrl-d/Ctrl-u/Ctrl-y and other control-letter variants are inert while Ctrl-c and Ctrl-i remain the only intentional control-key paths; numbered shortcuts ignore Control/Alt and stay inactive while settings owns input |
| Settings and config persistence | `docs/2026-06-01-ghzinga-settings-config-plan.md` documents `~/.config/ghzinga/config.toml`, `XDG_CONFIG_HOME`, `GZG_CONFIG_PATH`, CLI override precedence, and the in-app settings flow; config tests verify missing-file defaults, TOML parsing, invalid-value diagnostics, and save output; reducer/render tests verify `s`, footer `[⚙ settings]`, theme/symbol/spacing selection rows, and save intents; the tmux mouse-smoke capture clicks the `compact` settings row, verifies the saved-status message, records the capture-local TOML in the manifest, and deletes the generated config after capture |
| Comfortable and compact spacing modes | `docs/2026-06-01-ghzinga-spacing-density-plan.md` documents the Gmail-style density rules and gh-dash-like comfortable default; `ui.spacing`, `--spacing`, settings keyboard `p`, and settings rows support `comfortable` and `compact`; renderer tests verify compact keeps dense full-width rows while comfortable inserts section breathing room, repeated-row gaps, a gh-dash-like content gutter, equal chrome/content padding, fixed nav padding plus a continuous separator below the tab buttons, top/bottom content padding, overview timeline item padding above and below separators, and a wide-terminal readable-column cap for read-heavy tabs while leaving Files/diffs full-width; hit rectangles stay aligned to the visible rows |
| Symbol modes and font fallback | `default_symbol_mode_is_emoji` verifies emoji symbols are the default; `emoji_controls_keep_text_labels` verifies emoji controls still include readable text labels; `emoji_tabs_have_symbols` verifies the top navigation selectors carry emoji symbols; `ascii_symbols_are_plain_terminal_text` verifies the `--symbols ascii` fallback remains plain terminal text; `once_can_render_ascii_symbols_when_requested` verifies the CLI can opt into ASCII when a terminal or font cannot render emoji cleanly |
| Highlighted resource header | `header_identity_and_title_use_highlight_background` verifies the GitHub identity link and PR/issue title render on the theme `surface0` background instead of the normal panel background, while existing header hit-area tests keep the link clickable |
| Responsive TUI wrapping | `docs/2026-05-31-ghzinga-responsive-chrome-plan.md` documents the gh-dash-inspired wrapping strategy; `ViewRects::compute` tests verify narrow chrome row reservations, status-before-tabs ordering, and cramped content preservation; `narrow_render_wraps_chrome_without_losing_click_targets` verifies wrapped tabs/footer controls keep hit areas; status render tests verify loading messages use a separate detail row and do not displace the main summary; `oversized_status_pieces_wrap_without_early_ellipsis` verifies long status chips wrap before truncation; `header_wrap_keeps_identity_state_updated_and_title_visible` verifies wrapped header metadata remains visible; `resource_tab_bar_keeps_header_identity_and_title_visible` verifies multiple open resources do not hide the active GitHub link or PR/issue title; `resource_tab_bar_uses_single_gap_before_header_identity` verifies comfortable spacing does not stack two blank rows between resource tabs and the active resource header; `extremely_narrow_tabs_fit_visible_width` and `extremely_narrow_footer_controls_fit_visible_width` verify clipped labels keep hit areas aligned with visible controls; `content_rows_wrap_long_text_and_preserve_click_targets` verifies the final content wrapping pass keeps wrapped clickable rows clickable; `narrow_content_wraps_metadata_without_clipping` verifies long metadata wraps in a narrow viewport; `wrap_display_text` tests preserve markup characters while splitting wide/emoji text by display width |
| Footer message discipline | `footer_omits_verbose_shortcut_and_scroll_hint` verifies the footer no longer renders an always-on scroll/shortcut cheat sheet; the footer message area is reserved for loading, save, status, and error messages |
| Configurable draggable content scrollbar | `scrolling_reveals_transient_scrollbar` verifies scroll input makes the default on-scroll scrollbar visible and that it fades after the configured render countdown; `scrollbar_visibility_mode_controls_visibility` verifies always/hidden/on-scroll state behavior; `content_scrollbar_is_transient_after_scroll_input` verifies the Ratatui-rendered content pane has no thumb at rest, shows the thumb after scroll input, and hides it again after subsequent renders; `content_scrollbar_modes_control_rendering_and_hit_area` verifies always/hidden rendering and visible scrollbar hit-target registration; `mouse_scrollbar_click_and_drag_updates_scroll_position` verifies click/drag/release mapping from the right-edge track to `AppState.scroll`; `scrollbar_content_length_tracks_rows_not_only_scroll_limit`, `content_scrollbar_reaches_bottom_at_scroll_limit`, and `content_scrollbar_snaps_to_bottom_across_common_sizes` verify the Ratatui scrollbar uses rendered content length plus viewport height, maps the scroll offset onto Ratatui's position range, and snaps the endpoint cell to the top or bottom at scroll limits; PR/issue PageDown capture validators assert saved scroll captures include a visible scrollbar thumb |
| Narrow/medium/large UX rendering | regenerated tmux captures for `80x24`, `120x36`, `160x50` in PR and issue capture directories; `captures/ghzinga-pr-81834/capture_ghzinga.py --self-test` verifies the manifest parser rejects duplicate or unnamed size/frame entries and keeps narrow-terminal load detection tolerant of truncated long titles, `captures/ghzinga-pr-81834/capture_ghzinga.py --validate-only` verifies PR frame coverage, markers, footer controls, generated files, actual tmux dimensions, capture-local `GZG_CONFIG_PATH` isolation, PR body/comment/review/commit/check/file/link content markers, and that app/rendering source paths have not changed since the manifest revision or recorded app/rendering tree hash; the same script with `--root captures/ghzinga-issue-88499 --mode issue --validate-only` verifies issue frames plus body/comment/link content markers and rejects manifests that are not backed by the expected issue URL, title, and `fixtures/issue-88499.json`; `scripts/update-capture-manifests.py --check` verifies every ghzinga capture manifest is stamped with the current app/rendering tree hash so squash merges and capture-only commits do not break CI when the rendered app tree is unchanged; CI runs the self-test and both validate-only checks |
| Visible URL browser open and copy | reducer tests for `o` and `y`; `ResourceId::web_url` tests verify PR and issue URL construction; `keyboard_o_opens_first_visible_url`, `keyboard_o_falls_back_to_current_resource_url`, `keyboard_y_copies_first_visible_open_url`, `keyboard_y_copies_first_visible_navigation_target`, and `keyboard_y_falls_back_to_current_resource_url` verify app-level browser/copy behavior for visible content links and current-resource fallback; `url_open_command_uses_browser_env_when_available` and `url_open_command_preserves_browser_arguments` verify the direct browser adapter; `clipboard_command_uses_explicit_env_command`, `clipboard_command_prefers_wayland_when_available`, and `clipboard_command_uses_xclip_for_x11` verify the direct clipboard adapter; no `gh` shell-out is used for browser opening or clipboard copy |
...
the source revision, app/rendering tree hash, target, command, tab, keys, and
actual tmux dimensions for each frame. The main capture validator self-test rejects duplicate or unnamed
size/frame manifest entries so a malformed manifest cannot silently overwrite
evidence. Capture commands pin `GZG_CONFIG_PATH` to a missing capture-local file
so saved user settings cannot change the checked-in UX evidence. The current
marker and content check verifies:
...
- footer controls `[🔄 refresh]`, active-tab `[➕ expand ]` / `[➖ collapse]`,
  `[❔ help]`, `[⚙ settings]`, and `[⏻ quit]` appear in every checked size set
...
- Issue captures include the issue body, activity comments, and detected
  issue/comment links
- PR mouse smoke captures include real tmux clicks on an Overview `[➕ more]` /
  `[➖ less]` content control, the Files, Checks, and Links tabs, individual
  file-row and check-row expand/collapse controls, keyboard `a` on Files for
  current-tab expand/collapse-all, real footer clicks on
  `[➕ expand ]`, `[➖ collapse]`, `[🔄 refresh]`, `[❔ help]`,
  `[⚙ settings]`, and `[⏻ quit]`, and a real click on a linked issue row that
  reaches the navigation path
- Issue mouse smoke captures include real tmux clicks on issue body `[➕ more]` /
  `[➖ less]`, the Activity and Links tabs, Activity `[details]`, footer
  `[🔄 refresh]`, `[➕ expand ]`, `[➖ collapse]`, and `[⏻ quit]`, keyboard `a` on Overview for
  current-tab expand/collapse-all, and a real click on a linked issue row that
  reaches the navigation path
...
3. The tmux mouse-smoke captures send xterm SGR mouse click sequences and plain
   keyboard keys to the running TUI and validates the captured terminal frame
   after each input, including single-content expansion/collapse, individual
   file-row and check-row expansion/collapse, footer bulk expansion/collapse,
   keyboard `a` bulk expansion/collapse, footer refresh status routing in
   fixture mode, fixture-backed footer `[⬇ full]` routing, Activity
   `[details]` exact permalink opening for both PR and issue resources, keyboard
   copy/open first-visible-URL routing through capture-local adapter commands,
   tab switching, linked-resource navigation, Backspace history, footer
   help/settings overlay opening, settings-row persistence into capture-local
   TOML, and footer quit clicks that exit the tmux sessions.

Covered click targets:

- tabs
- body `[more]` / `[less]`
- footer tab-level `[➕ expand ]` / `[➖ collapse]`
...
cargo run --bin gzg -- https://github.com/openclaw/openclaw/issues/88499 --tab activity --once
cargo run --bin gzg -- openclaw/openclaw#81834 --tab commits --once
cargo run --bin gzg -- openclaw/openclaw#81834 --tab checks --once
cargo run --bin gzg -- openclaw/openclaw#81834 --tab activity --once
GZG_LIVE_REQUIRE_PUBLIC_FALLBACK=1 scripts/live-smoke.sh
```
