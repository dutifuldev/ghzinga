...
| Comfortable and compact spacing modes | `docs/2026-06-01-ghzinga-spacing-density-plan.md` documents the density rules; `ui.spacing`, `--spacing`, settings keyboard `p`, and settings rows support `comfortable` and `compact`; renderer tests verify compact keeps dense rows while comfortable inserts section breathing room after rules and repeated file rows for gh-dash-like readability |
