...
| Opt-in live smoke | `scripts/live-smoke.sh` builds `target/debug/gzg` if needed, renders live public PR Overview/Activity/Checks/Files tabs and a live public issue Overview tab through the normal direct GitHub HTTP path, and validates high-signal markers. It is not part of CI because network availability, public target state, and GitHub API quota are external inputs. `GZG_LIVE_PR_TARGET` and `GZG_LIVE_ISSUE_TARGET` can override the defaults. |
