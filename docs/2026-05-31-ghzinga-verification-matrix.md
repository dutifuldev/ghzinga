...
| Opt-in live smoke | `scripts/live-smoke.sh` builds `target/debug/gzg` if needed, renders live public PR Overview/Activity/Commits/Checks/Files/Links tabs and live public issue Overview/Activity/Links tabs through the normal direct GitHub HTTP path, then reruns public PR and issue Overview with blank `GH_TOKEN`/`GITHUB_TOKEN` and `PATH` pointed at an empty directory so the app cannot use `gh auth token`. It validates high-signal markers in both authenticated-or-existing-token mode and no-login public fallback mode; no-login public fallback is skipped by default if GitHub's shared unauthenticated IP quota is exhausted, and `GZG_LIVE_REQUIRE_PUBLIC_FALLBACK=1` makes that condition fatal when strict fallback evidence is required. It is not part of CI because network availability, public target state, and GitHub API quota are external inputs. `GZG_LIVE_PR_TARGET` and `GZG_LIVE_ISSUE_TARGET` can override the defaults. |
