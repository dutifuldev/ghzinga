...
| Comfortable and compact spacing modes | `docs/2026-06-01-ghzinga-spacing-density-plan.md` documents the Gmail-style density rules and gh-dash-like comfortable default; `ui.spacing`, `--spacing`, settings keyboard `p`, and settings rows support `comfortable` and `compact`; renderer tests verify compact keeps dense full-width rows while comfortable inserts section breathing room, repeated-row gaps, a gh-dash-like content gutter, and a wide-terminal readable-column cap for read-heavy tabs while leaving Files/diffs full-width; hit rectangles stay aligned to the visible rows |
