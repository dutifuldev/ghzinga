...
| Opt-in live smoke | `scripts/live-smoke.sh` builds `target/debug/gzg` if needed, renders live public PR Overview/Activity/Commits/Checks/Files/Links tabs and live public issue Overview/Activity/Links tabs through the normal direct GitHub HTTP path, and validates high-signal markers. It is not part of CI because network availability, public target state, and GitHub API quota are external inputs. `GZG_LIVE_PR_TARGET` and `GZG_LIVE_ISSUE_TARGET` can override the defaults. |
