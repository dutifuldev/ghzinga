...
| Mouse click routing | synthetic Crossterm `MouseEvent` tests in `src/app/update.rs`, render-to-click tests in `src/render/resource.rs`, and PR/issue evidence under `captures/ghzinga-pr-81834/mouse-smoke/` and `captures/ghzinga-issue-88499/mouse-smoke/` generated by sending real xterm SGR mouse events into the app inside tmux; the PR smoke run uses fixture-backed linked-resource loading to prove a clicked issue row replaces the current TUI view and Backspace restores the original PR, while the issue smoke run proves issue body expansion/collapse, Activity `[details]` exact URL opening, footer copy/open first-visible-URL routing, linked-issue navigation, and Backspace restoration; the validators reject duplicate or unnamed capture frames so a broken manifest cannot silently overwrite click evidence |
...
- Issue mouse smoke captures include real tmux clicks on issue body `[+ more]` /
  ...
  `[refresh]`, `[copy]`, `[open]`, and `[quit]`, and a real click on a linked
...
3. The tmux mouse-smoke captures send xterm SGR mouse click sequences to the
   ...
   status routing in fixture mode, Activity `[details]` exact permalink opening
   for both PR and issue resources, footer copy/open first-visible-URL routing
   ...
   settings-row persistence into capture-local TOML, and footer quit clicks that
