...
| Comfortable and compact spacing modes | `ui.spacing`, `--spacing`, settings keyboard `p`, and settings rows support `comfortable` and `compact`; renderer tests verify compact keeps dense rows while comfortable inserts section breathing room after rules for gh-dash-like readability |
