...
- PR mouse smoke captures include real tmux clicks on an Overview `[➕ more]` /
  ...
  `[➕ all]`, `[➖ all]`, `[🔄 refresh]`, `[📋 copy]`, `[🌐 open]`, `[❔ help]`,
...
- Issue mouse smoke captures include real tmux clicks on issue body `[➕ more]` /
  ...
  `[🔄 refresh]`, `[📋 copy]`, `[🌐 open]`, and `[⏻ quit]`, keyboard `a` on Overview for
...
3. The tmux mouse-smoke captures send xterm SGR mouse click sequences and plain
   ...
   `[details]` exact permalink opening for both PR and issue resources, footer
