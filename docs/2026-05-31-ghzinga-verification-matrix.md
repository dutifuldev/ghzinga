...
| No special UI fonts required | `default_symbol_mode_is_ascii` verifies ASCII symbols are the default; `ascii_symbols_are_plain_terminal_text` verifies every default symbol label is ASCII text, including controls such as `[+ more]`, `[- less]`, `[refresh]`, `[copy]`, `[open]`, `[load full]`, `[quit]`, `[help]`, `OK`, `!!`, `..`, and `*`; continuous separator rules use standard terminal line glyphs and do not require Nerd Font icons; `emoji_controls_keep_text_labels` verifies opt-in emoji controls still include readable text labels, and `once_can_render_emoji_symbols_when_requested` verifies `--symbols emoji` is opt-in at the CLI |
...
3. The tmux mouse-smoke captures send xterm SGR mouse click sequences and plain
   ...
   fixture mode, fixture-backed footer `[load full]` routing, Activity
