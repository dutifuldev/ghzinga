...
- Rust unit tests for URL normalization, pane identity validation, session
  scoping, and the `gzg herdr-plugin open` entrypoint.
- Fake Herdr shell tests for open, reuse, stale-pane, and viewer launch behavior.
- `scripts/herdr-plugin-live-smoke.sh`, which starts a disposable Herdr session,
  links this plugin, opens a real right-side pane, and verifies ghzinga rendered
  fixture content there.
